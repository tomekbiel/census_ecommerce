# File: gen_faker/gen_ecom_data_optimized.py
import pandas as pd
import numpy as np
...
class EcommerceDataGenerator:
    def __init__(self, target_customers: int = 15000, target_revenue: float = 2300000,
        ...
        self.target_revenue = target_revenue  # €2.3 million
        ...
        self.base_total_customers = 2000000  # 2.0 million base from Shopify
        self.base_revenue_nov_2024 = 1750000000  # $1.75 billion Nov 2024

        self.customer_scaling_factor = self.target_customers / self.base_total_customers
        self.revenue_scaling_factor = self.target_revenue / self.base_revenue_nov_2024

        self.categories = self._load_category_distribution()
        self.email_cache = {}  # Cache for generated emails
    ...
    def _load_monthly_stats(self) -> Dict:
        """Load monthly statistics from Shopify data"""
        monthly_stats = {}
        for _, row in self.shopify_data.iterrows():
            key = (row['year'], row['month'])
            monthly_stats[key] = {
                'sales_weight': row['Sales_Weight'],
                'active_stores': row['Active stores (mln)'] * 1e6,
                'repeat_rate': row['Est. Customer repeat rate (orders/customer)'],
                'avg_order_value': row['Avg. order value (USD)'] * 0.9,  # Convert to EUR approx
    ...
    def _load_category_distribution(self) -> Dict[str, Dict]:
        """Load category distribution from Shopify data with proper ranking"""
        # Define category hierarchy based on sales patterns
        category_hierarchy = {
    ...
    def _get_category_based_on_month(self, year: int, month: int) -> str:
        """Get category based on monthly top categories from Shopify data"""
        key = (year, month)
        if key in self.monthly_stats:
            top_categories = self.monthly_stats[key]['top_categories']
            if top_categories:
                # Use the first category from top categories list
                ...
                for cat, stats in self.categories.items():
                    if cat.lower() in [c.lower() for c in top_categories]:
                        ...
                        # Higher weight for categories that appear earlier in top list
    ...
    def _generate_email_from_name(self, name: str) -> str:
        """Generate email address based on customer name"""
        # Clean the name and create email-friendly version
        clean_name = re.sub(r'[^a-zA-Z\s]', '', name).lower().strip()
        parts = clean_name.split()

        if len(parts) >= 2:
            # First format: john.doe@email.com
    ...
    def _get_monthly_parameters(self, year: int, month: int) -> Dict:
        """Get monthly parameters from Shopify data"""
        key = (year, month)
        if key in self.monthly_stats:
    ...
    def generate_products(self, num_products: int = 200) -> pd.DataFrame:
        """Generate optimized product catalog"""
        products = []

        for i in range(1, num_products + 1):
            # Create products across different creation dates from 2018-2024
            ...
            # Price based on category rank (higher rank = higher price)
            base_multiplier = 0.8 + (0.4 * (7 - category_stats['rank']) / 6)  # Higher rank = higher price
    ...
    def generate_customers(self) -> pd.DataFrame:
        """Generate optimized customer data"""
        customers = []

        # Distribute customers across years 2018-2024 based on estimated customers from Shopify
        yearly_estimation = {}
        for year in range(2018, 2025):
            year_data = [stats for (y, m), stats in self.monthly_stats.items() if y == year]
            if year_data:
                avg_customers = np.mean([data['estimated_customers'] for data in year_data])
                yearly_estimation[year] = avg_customers
            else:
                yearly_estimation[year] = 1000000  # Default

        total_estimated = sum(yearly_estimation.values())

        customer_id = 1
        for year in range(2018, 2025):
            num_customers = int(self.target_customers * (yearly_estimation[year] / total_estimated))
    ...
    def generate_orders(self, customers_df: pd.DataFrame, products_df: pd.DataFrame) -> pd.DataFrame:
        """Generate orders with all item data included in one file for 2018-2024"""
        orders = []
        order_id = 1
        ...
        for date in tqdm(date_range, desc="Generating orders 2018-2024"):
            ...
            # Customers who joined before this date
            eligible_customers = customers_df[customers_df['join_date'] <= date.date()]
            if len(eligible_customers) == 0:
                continue

            # Calculate daily orders based on Shopify parameters
            scaling = monthly_params['sales_weight'] * self.customer_scaling_factor
            daily_orders = int((len(eligible_customers) * monthly_params['repeat_rate'] / 365) * scaling)
            daily_orders = max(1, min(daily_orders, 200))
            ...
            for _ in range(daily_orders):
                ...
                num_items = min(np.random.geometric(p=0.35), 8)
                order_total = 0.0
                total_quantity = 0
                total_discount = 0.0
                categories = []
                ...
                for _ in range(num_items):
                    # Bias product selection towards current month's top categories
                    ...
                    quantity = max(1, np.random.poisson(lam=1.3))
    ...
    def generate_all_data(self):
        """Generate all datasets and return as DataFrames"""
...
def main():
    print("Starting optimized e-commerce data generation...")
    print("Target: 15,000 customers, €2.3M revenue (2018-2024 data)")
    ...
    avg_aov = completed_orders['total_amount'].mean()
    ...
    print(f"Cancelled Orders: {len(orders_df[orders_df['status'] == 'cancelled']):,}")
    ...
    yearly_stats = orders_df[orders_df['status'] == 'completed'].groupby('year').agg({
    ...
    print(f"\nGenerated file structure (3 files):")
    print(f"1. products.csv - {len(products_df):,} products")
    print(f"2. customers.csv - {len(customers_df):,} customers")
    print(f"3. orders.csv - {len(orders_df):,} orders")

    print(f"\nData includes years 2018-2024 with Shopify distribution")
